...
class KaggleConfig(Config):
    ...
    IMAGES_PER_GPU = 2  # 2 modified by Hubert

    # Number of classes (including background)
    NUM_CLASSES = 8  # Background + instances
    ...
    BACKBONE = "resnet101"   # Modified by Hubert (originally this line was turned off)

    # Input image resizing
    # Random crops of size 512x512
    IMAGE_RESIZE_MODE = "square"
    IMAGE_MIN_DIM = 1024   # modified by Hubert (originally 1024)
    IMAGE_MAX_DIM = 1024
    IMAGE_MIN_SCALE = 2.0
...
###############################################
    ...
    #MAX_GT_INSTANCES = 40    ## turned on by Hubert
...
class KaggleInferenceConfig(KaggleConfig):
    ...
    GPU_COUNT = 1 # 1 modified by Hubert (5/28)
    IMAGES_PER_GPU = 2
...
#        self.add_class("kaggle", 0, "others")
        ...
        #self.add_class("kaggle", 0, "others")      # modified by Hubert to prevent errors due to images with no instance
    ...
    # the folder of train_color contains 15k images 
        ...
        subset_dir = "train_color_15k/" if subset in ["train","val"] else "test_color/"
        #subset_dir = "train_color/" if subset in ["train","val"] else "test/"
    
    # the folder of train_color contains 15k images 
        dataset_dir = os.path.join(dataset_dir, subset_dir)

        TRAIN_IMAGE_IDS, VAL_IMAGE_IDS = train_test_split(os.listdir(dataset_dir), test_size=0.01, random_state=0) 
    #determine ratio later
...
###################### Coco "load_mask" (mutiple class/instances)######################
    ...
    def load_mask(self, image_id):
        ...
        if (len(mask)>0):
            ...
            return super(KaggleDataset, self).load_mask(image_id)
        #mask = np.stack(mask, axis=2).astype(np.bool)
        #class_ids = np.array(class_ids, dtype=np.int32)
